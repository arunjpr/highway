...
//         echo  $this->db->last_query();die;
    ...
    public function get_Vehicle_by_vehicle_id($vehicle_id) { 
        $result = $this->db->get_where($this->_vehicle, array('Id' => $vehicle_id , 'deletion_status' => 0)); 
        return $result->row_array(); 
    } 

    public function published_vehicle_by_id($vehicle_id) { 
        $this->db->update($this->_vehicle, array('Status' => 1), array('Id' => $vehicle_id));  
        return $this->db->affected_rows(); 
    } 

    public function unpublished_vehicle_by_id($vehicle_id) { 
        $this->db->update($this->_vehicle, array('Status' => 0), array('Id' => $vehicle_id)); 
        return $this->db->affected_rows(); 
    } 

    public function update_vehicle($vehicle_id, $data) { 
        $this->db->update($this->_vehicle, $data, array('Id' => $vehicle_id)); 
        return $this->db->affected_rows(); 
    } 
	
    public function remove_vehicle_by_id($vehicle_id) { 
        $this->db->update($this->_vehicle, array('deletion_status' => 1), array('Id' => $vehicle_id)); 
        return $this->db->affected_rows(); 
    } 
