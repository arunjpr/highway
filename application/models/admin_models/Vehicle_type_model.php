...
class Vehicle_type_model extends CI_Model {
   ...
   public function __construct() {
    ...
    private $_vehicle_type_type = 'vehicle';  
...
//         echo  $this->db->last_query();die;
    ...
    public function get_vehicle_type_info() { 
        $this->db->select('*') 
                ...
                ->where(array("tbl_vehicle_type.v_t_status" => 1,"tbl_vehicle_type.v_t_delete" => 0));
        $query_result = $this->db->get(); 
    ...
    public function get_vehicle_type_by_id($vehicle_id) { 
        $result = $this->db->get_where($this->_vehicle_type, array('v_Id' => $vehicle_id , 'v_status' => 1)); 
        return $result->row_array(); 
    } 

    public function active_vehicle_by_id($vehicle_id) { 
        $this->db->update($this->_vehicle_type, array('v_status' => 1), array('v_Id' => $vehicle_id));  
        return $this->db->affected_rows(); 
    } 

    public function inactive_vehicle_by_id($vehicle_id) { 
        $this->db->update($this->_vehicle_type, array('v_status' => 0), array('v_Id' => $vehicle_id)); 
        return $this->db->affected_rows(); 
    } 

    public function update_vehicle_type($vehicle_id, $data) { 
        $this->db->update($this->_vehicle_type, $data, array('v_Id' => $vehicle_id)); 
        return $this->db->affected_rows(); 
    } 
	
    public function remove_vehicle_type($vehicle_id) { 
        $this->db->update($this->_vehicle_type, array('v_delete' => 1), array('v_Id' => $vehicle_id)); 
        return $this->db->affected_rows(); 
    }   
}
